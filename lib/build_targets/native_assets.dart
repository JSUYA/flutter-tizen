...
class TizenFlutterNativeAssetsBuildRunnerImpl extends FlutterNativeAssetsBuildRunnerImpl {
  ...
  Future<void> setCCompilerConfig(CodeAssetTarget target) async {
    if (target is AndroidAssetTarget) {
      target.cCompilerConfigSync = await tizenCCompilerConfigLinux(
        fileSystem: fileSystem,
...
@visibleForTesting
Future<CCompilerConfig?> tizenCCompilerConfigLinux({
  ...
  if (whichResult.exitCode != 0) {
    if (throwIfNotFound) {
      throwToolExit('Failed to find $clangPpBinary on PATH.');
    }
    return null;
  }
  ...
  if (linker == null && throwIfNotFound) {
    throwToolExit(
      ...
      'Install the lld or binutils package to provide a linker.',
    );
  }
